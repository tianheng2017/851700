...
class Distribut extends Base {
    ...
    public function distributor_list(){
    	...
    	foreach ($user_list as $k=>$val){
			$uids = find_all($members, $val['user_id']);
			...
			$user_list[$k]['distribut_money'] = Db::name('rebate_log')->where(['user_id' => $val['user_id'], 'status' => 3])->sum('money');
			$user_list[$k]['team_yj'] = Db::name('order')->whereIn("user_id", $uids)->where(['order_status' => 2, 'pay_status' => 1])->sum('total_amount');
    	}
    ...
    public function goods_list(){
        ...
        $cache = tpCache('distribut');
    	$where = ' cat_id = ' . $cache['cat_id'];
    	$cat_id = I('cat_id/d');
        $bind = array();
    	if($cat_id > 0)
    	{
