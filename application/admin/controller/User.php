...
class User extends Base {
    ...
    public function ajaxindex(){
		...
		foreach ($userList as $k => $v){
			$dfscore = M('activity_pool')->where('user_id', $v['user_id'])->value('score');
			$userList[$k]['dfscore'] = isset($dfscore) ? $dfscore : 0;
		}
		$region = M('region')->column('name','id');
    ...
    public function detail(){
        ...
        if(IS_POST){
            ...
            $_POST['province_id'] = intval($_POST['province_id']);
			$_POST['city_id'] = intval($_POST['city_id']);
			$_POST['district_id'] = intval($_POST['district_id']);
			if (in_array($_POST['city_id'], [2,300,339,569,25487,31633]) && !$_POST['district_id']){
				exit($this->error('请选择第三个下拉菜单'));
			}
			$ids = M('users')->where([
			...
			count($ids) && count($ids)<>1 && !in_array($uid, $ids) && exit($this->error('此区域已有领导人，请重新选择'));
		...
		$c = M('region')->where(array('parent_id'=>isset($user['province_id'])?$user['province_id']:0,'level'=> 2))->select();
		$d = M('region')->where(array('parent_id'=>isset($user['city_id'])?$user['city_id']:0,'level'=> 3))->select();
	...
	public function edit_score_tx()
    {
        $id = I('id');
        $withdrawals = DB::name('score_tx')->where('id',$id)->find();
        $user = M('users')->where("user_id = {$withdrawals[user_id]}")->find();
        if (IS_POST) {
            $data = I('post.');
            // 如果是已经给用户转账 则生成转账流水记录
            if ($data['status'] == 1 && $withdrawals['status'] != 1) {
                if ($user['pay_points'] < $withdrawals['money']) {
                    $this->error("用户鱼分不足".$withdrawals['money']."，不够提现");
                    exit;
                }
                accountLog($withdrawals['user_id'], 0, ($withdrawals['money'] * -1), "鱼分提现");
                $score_remittance = array(
                    'user_id' => $withdrawals['user_id'],
                    'bank_name' => $withdrawals['bank_name'],
                    'account_bank' => $withdrawals['account_bank'],
                    'account_name' => $withdrawals['account_name'],
                ...
                M('score_remittance')->add($score_remittance);
            }
            DB::name('score_tx')->update($data);
            $this->success("操作成功!", U('Admin/User/score_remittance'), 3);
            exit;
        }
        ...
        elseif ($user['mobile'])
            $withdrawals['user_name'] = $user['mobile'];
		$this->assign('fee', tpCache('distribut.s4'));
        $this->assign('user', $user);
        $this->assign('data', $withdrawals);
        return $this->fetch();
    }
	...
	public function score_tx_update(){
        $id = I('id/a');
        $status = I('status');
        $withdrawals = M('score_tx')->where('id','in', $id)->select();
        if($status == 1){
            $r = M('score_tx')->where('id','in', $id)->save(array('status'=>$status,'check_time'=>time()));
        }else if($status == -1){
            $r = M('score_tx')->where('id','in', $id)->save(array('status'=>$status,'refuse_time'=>time()));
        }else if($status == 3){
            $r = M('score_tx')->where('id in ('.implode(',', $id).')')->delete();
        }else{
            accountLog($val['user_id'], 0, ($val['money'] * -1),"管理员处理用户鱼分提现申请");//手动转账，默认视为已通过线下转方式处理了该笔提现申请
            $r = M('score_tx')->where('id in ('.implode(',', $id).')')->save(array('status'=>2,'pay_time'=>time()));
        }
        if($r){
            $this->ajaxReturn(array('status'=>1,'msg'=>"操作成功"),'JSON');
        }else{
	...
	public function score_remittance(){
        $model = M("score_remittance");
        $_GET = array_merge($_GET,$_POST);
        unset($_GET['create_time']);
	...
	public function give_score(){
		$count = M("activity_pool")->count();
		$Page  = new Page($count, 16);
        $list = M("activity_pool")->order("`id` desc")->limit($Page->firstRow.','.$Page->listRows)->select();
 		foreach ($list as $k => $v) {
			$list[$k]['mobile'] = M('users')->where('user_id', $v['user_id'])->value('nickname');
		}
		$this->assign('pager',$Page);
