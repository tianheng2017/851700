...
function realSendSMS($mobile, $smsSign, $smsParam , $templateCode, $code=null)
{
	$config = tpCache('sms');
	$res = json_decode($smsParam, true);
	$smsapi = "http://api.smsbao.com/";
	$user = $config['sms_appkey'];
	$pass = md5($config['sms_secretKey']);
	$content="【".$res['product']."】您的验证码是".$res['code'];
	$phone = $mobile;
	...
	if($result == '0'){
		...
		return array('status' => -1, 'msg' => 'Code|500'.print_r($result,true));
	}
...
//     */
function realSendSMS222($mobile, $smsSign, $smsParam , $templateCode)
{
...
function activity_log($user_id, $money){
    $update = Db::name('users')->where('user_id', $user_id)->update(['activity_money' => ['exp', 'activity_money+'.$money]]);
    if($update){
        return true;
    }else{
        return false;
...
function confirm_order($id,$user_id = 0){
    ...
    if(!$row) return array('status'=>-3,'msg'=>'操作失败');
	$goods = Db::view('order','order_id')
			->view('order_goods', 'goods_id', 'order.order_id = order_goods.order_id')
			->view('goods', 'cat_id', 'order_goods.goods_id = goods.goods_id')
			->where('order_id', $order['order_id'])
			->find();
	
	if ($config['cat_id'] == $goods['cat_id']){
		//activity_pool($order);
		tgjl_count($order);
		tdjl_count($order);
		qyjl_count($order);
...
function qyjl_count($order){
	$config = tpCache('distribut');
	if ($config['switch']){
		$leader_ids = [];
		$qx_ids = [];
		$sj_ids = [];
		$zxs = [1,338,10543,31929];
		$tsid = [2,300,339,569,25487,31633];
		$cat = M('users')->where('district_id|city_id|province_id','<>',0)->field('user_id,district_id,city_id,province_id')->select();
		if (count($cat)){
			foreach ($cat as $v){
				if ($v['district_id']){
					$qx_ids[$v['district_id']] = $v['user_id'];
				}else if ($v['city_id'] && !in_array($v['city_id'], $tsid)){
					$sj_ids[$v['city_id']] = $v['user_id'];
				}else if ($v['province_id'] && in_array($v['province_id'], $zxs)){
					$sj_ids[$v['province_id']] = $v['user_id'];
				}
			}
		}
		$user = M('users')->where('user_id', $order['user_id'])->find();
		if ($user['district_reg']){
			$level = M('region')->where('id', $user['district_reg'])->value('level');
			if ($level == 3 && count($qx_ids)){
				if (array_key_exists($user['district_reg'], $qx_ids)){
					$leader_uid = $qx_ids[$user['district_reg']];
					$data = [
						'user_id' => $qx_ids[$user['district_reg']],
						'level'	  => 3,
					];
					$leader_ids[] = $data;
				}
		...
		if ($user['city_reg']){
			$level = M('region')->where('id', $user['city_reg'])->value('level');
			if (($level == 2 && !in_array($user['city_reg'], $tsid)) && count($sj_ids)){
				if (array_key_exists($user['city_reg'], $sj_ids)){
					$leader_all = array_column($leader_ids, 'user_id');
					$leader_uid = $sj_ids[$user['city_reg']];
					if (!in_array($leader_uid, $leader_all)){
						$data = [
							'user_id' => $sj_ids[$user['city_reg']],
							'level'	  => 2,
						];
						$leader_ids[] = $data;
					}
		...
		if ($user['province_reg']){
			$level = M('region')->where('id', $user['province_reg'])->value('level');
			if (($level == 2 || in_array($user['province_reg'], $zxs)) && count($sj_ids)){
				if (array_key_exists($user['province_reg'], $sj_ids)){
					$leader_all = array_column($leader_ids, 'user_id');
					$leader_uid = $sj_ids[$user['province_reg']];
					if (!in_array($leader_uid, $leader_all)){
						$data = [
							'user_id' => $sj_ids[$user['province_reg']],
							'level'	  => 2,
						];
						$leader_ids[] = $data;
					}
		...
		if(count($leader_ids)){
			$sjjl = $order['goods_price'] * $config['s14'] / 100;
			$qxjl = $order['goods_price'] * $config['s15'] / 100;
			foreach ($leader_ids as $v){
				if ($v['level'] == 2){
					$txt1 = "市级领导人奖励(".$user['mobile'].")";
					activity_log($v['user_id'], $sjjl);
					rebateLog($v['user_id'], $sjjl, $order, $txt1);
				}
				if ($v['level'] == 3){
					$txt1 = "区县领导人奖励(".$user['mobile'].")";
					activity_log($v['user_id'], $qxjl);
					rebateLog($v['user_id'], $qxjl, $order, $txt1);
				}
				$now_uid = $v['user_id'];
				do{
					$first_leader = M('users')->where('user_id', $now_uid)->value('first_leader');
					if ($first_leader){
						$leader = M('users')->where('user_id', $first_leader)->where('province_id|city_id|district_id', '<>', 0)->find();
						if ($leader){
							$ssjl = $order['goods_price'] * $config['s16'] / 100;
							$txt1 = "领导人推荐领导人奖励(".$user['mobile'].")";
							activity_log($first_leader, $ssjl);
							rebateLog($first_leader, $ssjl, $order, $txt1);
							$flag = false;
						}else{
							$now_uid = $first_leader;
							$flag = true;
						}
					}else{
						$flag = false;
...
function rebateLog($uid, $money, $order, $remark){
	$nickname = M('users')->where('user_id', $order['user_id'])->value('nickname');
	$data = [
		'user_id' 		=> $uid,
...
function find_all($members, $uid){
    ...
    do{
        ...
        $uids = array_column($temp, 'user_id');
    }while($state == true);
	return array_column($all, 'user_id');
}

function tdjl_count($order){
	$config = tpCache('distribut');
	if ($config['switch']){
		$arr1 = [];
		$arr2 = [];
		$arr3 = [];
		$user = M('users')->where('user_id', $order['user_id'])->find();
		$members = M('users')->field('user_id,first_leader')->select();
		$ids = array_column($members, 'user_id');
		foreach ($ids as $v){
			$xx = M('users')->where('first_leader', $v)->column('user_id');
			$count = count($xx);
			if ($count >= 20 && $count < 30){
				$gcount = M('activity_pool')->whereIn('user_id', $xx)->count();
				if ($gcount >= 20){
					$arr1[] = $v;
				}
		...
		if(count($arr1)){
			foreach ($arr1 as $v){
				$temp1 = find_all($members, $v);
				if (count($temp1) >= 100){
					if (in_array($order['user_id'], $temp1)){
						$money1 = $order['goods_price'] * $config['s11'] / 100;
						if ($money1){
							$txt1 = "1级团队奖励(".$user['mobile'].")";
							activity_log($v, $money1);
							rebateLog($v, $money1, $order, $txt1);
						}
					}
				}
			}
		...
		if(count($arr2)){
			foreach ($arr2 as $v){
				$temp2 = find_all($members, $v);
				if (count($temp2) >= 100 || count($temp2) < 200){
					if (in_array($order['user_id'], $temp2)){
						$money2 = $order['goods_price'] * $config['s11'] / 100;
						if ($money2){
							$txt2 = "1级团队奖励(".$user['mobile'].")";
							activity_log($v, $money2);
							rebateLog($v, $money2, $order, $txt2);
						}
					}
				}else if (count($temp2) >= 200){
					if (in_array($order['user_id'], $temp2)){
						$money2 = $order['goods_price'] * $config['s12'] / 100;
						if ($money2){
							$txt2 = "2级团队奖励(".$user['mobile'].")";
							activity_log($v, $money2);
							rebateLog($v, $money2, $order, $txt2);
						}
					}
		...
		if(count($arr3)){
			foreach ($arr3 as $v){
				$temp3 = find_all($members, $v);
				if (count($temp3) >= 100 && count($temp3) < 200){
					if (in_array($order['user_id'], $temp3)){
						$money3 = $order['goods_price'] * $config['s11'] / 100;
						if ($money3){
							$txt3 = "1级团队奖励(".$user['mobile'].")";
							activity_log($v, $money3);
							rebateLog($v, $money3, $order, $txt3);
						}
					}
				}else if (count($temp3) >= 200 && count($temp3) < 400){
					if (in_array($order['user_id'], $temp3)){
						$money3 = $order['goods_price'] * $config['s12'] / 100;
						if ($money3){
							$txt3 = "2级团队奖励(".$user['mobile'].")";
							activity_log($v, $money3);
							rebateLog($v, $money3, $order, $txt3);
						}
					}
				}else if (count($temp3) >= 400){
					if (in_array($order['user_id'], $temp3)){
						$money3 = $order['goods_price'] * $config['s13'] / 100;
						if ($money3){
							$txt3 = "3级团队奖励(".$user['mobile'].")";
							activity_log($v, $money3);
							rebateLog($v, $money3, $order, $txt3);
						}
					}
...
function tgjl_count($order){
	$config = tpCache('distribut');
	if ($config['switch']){
		$user = M('users')->where('user_id', $order['user_id'])->find();
		if ($user['first_leader']){
			$fidxxs = M('users')->where('first_leader', $user['first_leader'])->column('user_id');
			if (count($fidxxs) >= 1){
				$yes_fidxx_count = M('activity_pool')->whereIn('user_id', $fidxxs)->count();
				if ($yes_fidxx_count >= 1 && $yes_fidxx_count < 3){
					$tc1 = $config['s5']/100;
				}elseif($yes_fidxx_count >= 3 && $yes_fidxx_count < 5){
					$tc1 = $config['s6']/100;
				}elseif($yes_fidxx_count >= 5){
					$tc1 = $config['s8']/100;
				}else{
					$tc1 = 0;
				}
				$money1 = $order['goods_price'] * $tc1;
				if ($money1){
					$txt1 = "第1代提成(".$user['mobile'].")";
					activity_log($user['first_leader'], $money1);
					rebateLog($user['first_leader'], $money1, $order, $txt1);
				}
			}
		}
		if ($user['second_leader']){
			$yidxxs = M('users')->where('first_leader', $user['second_leader'])->column('user_id');
			if (count($yidxxs) >= 3){
				$yes_yidxx_count = M('activity_pool')->whereIn('user_id', $yidxxs)->count();
				if($yes_yidxx_count >= 3 && $yes_yidxx_count < 5){
					$tc2 = $config['s7']/100;
				}elseif($yes_yidxx_count >= 5){
					$tc2 = $config['s9']/100;
				}else{
					$tc2 = 0;
				}
				$money2 = $order['goods_price'] * $tc2;
				if ($money2){
					$txt2 = "第2代提成(".$user['mobile'].")";
					activity_log($user['second_leader'], $money2);
					rebateLog($user['second_leader'], $money2, $order, $txt2);
				}
			}
		}
		if ($user['third_leader']){
			$zidxxs = M('users')->where('first_leader', $user['third_leader'])->column('user_id');
			if (count($zidxxs) >= 5){
				$yes_zidxx_count = M('activity_pool')->whereIn('user_id', $zidxxs)->count();
				if($yes_zidxx_count >= 5){
					$tc3 = $config['s10']/100;
				}else{
					$tc3 = 0;
				}
				$money3 = $order['goods_price'] * $tc3;
				if ($money3){
					$txt3 = "第3代提成(".$user['mobile'].")";
					activity_log($user['third_leader'], $money3);
					rebateLog($user['third_leader'], $money3, $order, $txt3);
				}
			}
		}
	}
...
function activity_pool($order){
    $config = tpCache('distribut');
    if ($config['switch']){
        $num = $config['s1'] * $order['goods_price'];
        $pool = M('activity_pool')->where('user_id', $order['user_id'])->find();
        if ($pool){
            M('activity_pool')->where('user_id', $order['user_id'])->update(['score' => ['exp','score+'.$num], 'update_time' => time()]);
        }else{
            $data = [
            ...
            M('activity_pool')->add($data);
        }
    }
}

function consume_activity(){
    $config = tpCache('distribut');
    if ($config['switch']){
        $data = Db::view('activity_pool','*')
                ->view('users','user_id','users.user_id=activity_pool.user_id')
                ->where('activity_pool.score', '>', 0)
                ->where('activity_pool.status', 1)
                ->select();
        foreach ($data as $k => $v){
            $give_score = $v['score'] * $config['s2'] / 100;
            if ($give_score >= 1){
                if ($v['score'] - $give_score < 1){
                    $end_score = 0;
                }else{
                    $end_score = $v['score'] - $give_score;
                }
                M('activity_pool')->where('user_id', $v['user_id'])->save(['score' => $end_score, 'update_time' => time()]);
				accountLog($v['user_id'], 0, $give_score, "鱼分返还活动");
            }
        }
        return 'success';
    }
    ...
    for ($i = 0; $i < $prom_order_count; $i++) {
        if (array_key_exists($user_level, array_flip(explode(',', $prom_orders[$i]['group'])))) {
            ...
            if ($prom_order['type'] == 2) {
                accountLog($order['user_id'], 0, $prom_order['expression'], "订单活动赠送鱼分");
            }
            break;
        }
    }
    $points = M('order_goods')->where("order_id", $order['order_id'])->sum("give_integral * goods_num");
    $points && accountLog($order['user_id'], 0, $points, "下单赠送鱼分", 0, $order['order_id'], $order['order_sn']);
}
...
{
//    $parse_type = array('0'=>'满额打折','1'=>'满额优惠金额','2'=>'满额送倍数鱼分','3'=>'满额送优惠券','4'=>'满额免运费');
    $now = time();
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $use_percent_point = tpCache('shopping.point_use_percent') / 100;     //最大使用限制: 最大使用鱼分比例, 例如: 为50时, 未50% , 那么鱼分支付抵扣金额不能超过应付金额的50%
    ...
    if(($pay_points > 0 && $use_percent_point == 0) ||  ($pay_points >0 && $result['order_integral']==0)){
        return array('status' => -1, 'msg' => "该笔订单不能使用鱼分", 'result' => '鱼分'); // 返回结果状态
    }

    if ($pay_points && ($pay_points > $user['pay_points']))
        return array('status' => -5, 'msg' => "你的账户可用鱼分为:" . $user['pay_points'], 'result' => ''); // 返回结果状态
    ...
    $user_money = ($user_money > $order_amount) ? $order_amount : $user_money;  // 余额支付原理等同于鱼分
    $order_amount = $order_amount - $user_money; //  余额支付抵应付金额

    /*判断能否使用鱼分
     1..鱼分低于point_min_limit时,不可使用
     2.在不使用鱼分的情况下, 计算商品应付金额
     3.原则上, 鱼分支付不能超过商品应付金额的50%, 该值可在平台设置
     @{ */
    $point_rate = tpCache('shopping.point_rate'); //兑换比例: 如果拥有的鱼分小于该值, 不可使用
    $min_use_limit_point = tpCache('shopping.point_min_limit'); //最低使用额度: 如果拥有的鱼分小于该值, 不可使用


    if ($min_use_limit_point > 0 && $pay_points > 0 && $pay_points < $min_use_limit_point) {
        return array('status' => -1, 'msg' => "您使用的鱼分必须大于{$min_use_limit_point}才可以使用", 'result' => ''); // 返回结果状态
    }
    // 计算该笔订单最多使用多少鱼分
    if(($use_percent_point !=1 ) && $pay_points > $result['order_integral']) {
       return array('status'=>-1,'msg'=>"该笔订单, 您使用的鱼分不能大于{$result['order_integral']}",'result'=>'鱼分'); // 返回结果状态
    }


    $pay_points = ($pay_points / tpCache('shopping.point_rate')); // 鱼分支付 100 鱼分等于 1块钱
    $pay_points = ($pay_points > $order_amount) ? $order_amount : $pay_points; // 假设应付 1块钱 而用户输入了 200 鱼分 2块钱, 那么就让 $pay_points = 1块钱 等同于强制让用户输入1块钱
    $order_amount = $order_amount - $pay_points; //  鱼分抵消应付金额

    $total_amount = $goods_price + $shipping_price;
    //订单总价  应付金额  物流费  商品总价 节约金额 共多少件商品 鱼分  余额  优惠券
    $result = array(
        ...
        'anum' => $anum, // 商品总共数量
        'integral_money' => $pay_points,  // 鱼分抵消金额
