...
class OrderLogic extends Model
{
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='')
    {
        ...
        $data = array(
            ...
            'integral'         =>($car_price['pointsFee'] * tpCache('shopping.point_rate')), //'使用鱼分',
            'integral_money'   =>$car_price['pointsFee'],//'使用鱼分抵多少钱',
        ...
        foreach($cartList as $key => $val)
        {
            ...
            $data2['give_integral']      = $goods['give_integral']; // 购买商品赠送鱼分
        ...
        // 如果应付金额为0  可能是余额支付 + 鱼分 + 优惠券 这里订单支付状态直接变成已支付
        if($data['order_amount'] == 0)
        {
			$norder = M('Order')->where("order_id", $order_id)->find();
			activity_pool($norder);
        ...
        if($car_price['pointsFee']>0)
            M('Users')->where("user_id", $user_id)->setDec('pay_points',($car_price['pointsFee'] * tpCache('shopping.point_rate'))); // 消费鱼分
...
//        M('goods_activity')->where(array('act_id'=>$act_id))->setInc('act_count',$pre_sell_price['goods_num']);
        ...
        $data2['give_integral']      = $goods_activity['integral']; // 购买商品赠送鱼分
