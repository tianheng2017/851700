...
class UsersLogic extends Model
{
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$province_reg,$city_reg,$district_reg){
    	$is_validated = 0 ;
        ...
        $map['token'] = md5(time().mt_rand(1,99999));
		$map['province_reg'] = $province_reg;
		$map['city_reg'] = $city_reg;
		$map['district_reg'] = $district_reg;
		//exit(print_r($map,true));
        ...
        $pay_points = tpCache('basic.reg_integral'); // 会员注册赠送鱼分
        if($pay_points > 0)
            accountLog($user_id, 0,$pay_points, '会员注册赠送鱼分'); // 记录日志流水
        // 会员注册送优惠券
        $coupon = M('coupon')->where("send_end_time > ".time()." and ((createnum - send_num) > 0 or createnum = 0) and type = 2")->select();
        if(!empty($coupon)){
        	foreach ($coupon as $key => $val)
    ...
    public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
        ...
        if($order['user_money'] > 0 || $order['integral'] > 0){
            accountLog($user_id,$order['user_money'],$order['integral'],"订单取消，退回{$order['user_money']}元,{$order['integral']}鱼分");
        }
