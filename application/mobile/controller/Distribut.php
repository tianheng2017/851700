...
class Distribut extends MobileBase {
    ...
    public function lower_list(){
        $user =$this->user;
        $count = Db::name('users')->where('first_leader', '=', $user['user_id'])->count();
        $page = new Page($count,C('PAGESIZE'));
        $lists = Db::name('users')
            ->field('nickname,user_id,distribut_money,reg_time,head_pic')
            ->where('first_leader', '=', $user['user_id'])
            ->limit("{$page->firstRow},{$page->listRows}")
            ->order('user_id desc')
            ->select();
		$members = Db::name('users')->field('first_leader, user_id')->select();
		$uids = find_all($members, $user['user_id']);
		$teamyj = Db::name('order')->whereIn("user_id", $uids)->where(['order_status' => 2, 'pay_status' => 1])->sum('total_amount');
		
		foreach ($lists as $k => $val){
			$my_uids = find_all($members, $val['user_id']);
			$lists[$k]['team_count'] = count($my_uids);
			$lists[$k]['zhi_count'] = Db::name('users')->where('first_leader', $val['user_id'])->count();
			$lists[$k]['team_yj'] = Db::name('order')->whereIn("user_id", $my_uids)->where(['order_status' => 2, 'pay_status' => 1])->sum('total_amount');
		}
		
		$all = count($uids);
		$this->assign('all', $all); //团队人数
        $this->assign('count', $count);// 直推人数
		$this->assign('teamyj', $teamyj);// 团队业绩
        $this->assign('page', $page->show());// 分页
        $this->assign('lists',$lists); // 直推列表
        if(I('is_ajax'))
        {
    ...
    public function qr_code(){
        $ShareLink = urlencode("http://{$_SERVER[HTTP_HOST]}/Mobile/User/reg/first_leader/{$this->user_id}");
