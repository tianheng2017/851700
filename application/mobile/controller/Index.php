...
class Index extends MobileBase {
    ...
    public function getNotice()
    {
        if (session('?user')){
            $user = session('user');
            $old_id = cache('notice_'.$user['user_id']);
            $notice = M('article')->where('cat_id = 92 and is_open = 1')->field('article_id,title,content,add_time')->order('add_time desc')->limit(1)->find();
            if ($old_id <> $notice['article_id']){
                if($notice){
                    cache('notice_'.$user['user_id'], $notice['article_id']);
