...
class User extends MobileBase
{
    ...
    */
    public function _initialize()
    {
        ...
        $nologin = array(
            'timer', 'login', 'pop_login', 'do_login', 'logout', 'verify', 'set_pwd', 'finished',
    ...
    public function index()
    {
		...
		$this->assign('is_sign', $today['is_sign']);
		...
		$dfscore = M('activity_pool')->where('user_id', $this->user['user_id'])->value('score');
		$this->assign('dfscore', isset($dfscore) ? $dfscore : 0);
		$hhr = $this->user['province_id'] || $this->user['city_id'] || $this->user['district_id'];
		$this->assign('hhr', $hhr);
        return $this->fetch();
    }
...
//            $cartLogic->login_cart_handle($this->session_id, $res['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    public function reg()
    {
		...
		$first_leader = intval(I('get.first_leader'));
		if ($first_leader && M('users')->where('user_id', $first_leader)->count()){
			cookie('first_leader', $first_leader);
		}
        ...
        if (IS_POST) {
			...
			$province_reg = I('post.province');
			$city_reg = I('post.city');
			$district_reg = I('post.district');
            ...
            $data = $logic->reg($username, $password, $password2, $province_reg, $city_reg, $district_reg);
            if ($data['status'] != 1)
                $this->error($data['msg']);
...
//            $this->success('申请成功,客服第一时间会帮你处理', U('Mobile/User/order_list'));
	...
	public function activity_score(){
		if (IS_POST) {
			$data = I('post.');
			if (!ctype_digit($data['money'])){
				$this->error('提取额度必须为整数');
                exit;
			}
			if ($data['money'] < 100) {
                $this->error('单次最少提取100元');
                exit;
            }
			if ($data['money'] % 100 <> 0) {
                $this->error('提取额度必须为100元的整数倍');
                exit;
            }
            if ($data['money'] > $this->user['activity_money']) {
                $this->error("收益不足");
                exit;
            }
			$res = Db::name('users')->where('user_id', $this->user_id)->update(['activity_money' =>	['exp','activity_money-'.$data['money']]]);
			if ($res){
				accountLog($this->user_id,0,$data['money'],'收益提取');
				$this->success('提取成功，鱼分增加'.$data['money']);
                exit;
			}else{
				$this->error('提取失败');
                exit;
		...
		$this->assign('activity_money', $this->user['activity_money']?$this->user['activity_money']:0);
		return $this->fetch();
	}
	
	public function score_tx()
    {
        if (IS_POST) {
			$logic = new UsersLogic();
            $data = I('post.');
			...
			$file = request()->file('account_bank');
            $save_url = 'public/upload/comment/' . date('Y', time()) . '/' . date('m-d', time());
			$info = $file->rule('uniqid')->validate(['size' => 1024 * 1024 * 3, 'ext' => 'jpg,png,gif,jpeg'])->move($save_url);
			if ($info) {
				$comment_img = '/'.$save_url . '/' . $info->getFilename();
			...
			$data['account_bank'] = $comment_img;
			$data['account_name'] = '二微码收款';
			$data['bank_name'] = '微信/支付宝';
            $data['user_id'] = $this->user_id;
            $data['create_time'] = time();
            $distribut_min = tpCache('distribut.s3'); // 最少提现额度
			if (!ctype_digit($data['money'])){
				$this->error('提现数量必须为整数');
                exit;
			}
            if ($data['money'] < $distribut_min) {
                $this->error('每次最少提现' . $distribut_min);
                exit;
            }
            if ($data['money'] > $this->user['pay_points']) {
                $this->error("你最多可提现{$this->user['pay_points']}鱼分.");
                exit;
            }
			if ($data['money'] % 100 <> 0) {
                $this->error('提现额度必须为100的整数倍');
                exit;
            }
            $withdrawal = M('score_tx')->where(array('user_id' => $this->user_id, 'status' => 0))->sum('money');
            if ($this->user['pay_points'] < ($withdrawal + $data['money'])) {
                $this->error('您有提现申请待处理，本次提现鱼分不足');
				exit;
            }
            if (M('score_tx')->add($data)) {
                $this->success("已提交申请");
                exit;
            } else {
        ...
        $count = M('score_tx')->where($withdrawals_where)->count();
        $pagesize = C('PAGESIZE');
        $page = new Page($count, $pagesize);
        $list = M('score_tx')->where($withdrawals_where)->order("id desc")->limit("{$page->firstRow},{$page->listRows}")->select();
        ...
        $this->assign('pay_points', $this->user['pay_points']);    //用户余额
		$fee = tpCache('distribut.s4');
		$this->assign('fee', $fee);
		$this->assign('user', $this->user);
		$reg_sms_enable = tpCache('sms.regis_sms_enable');
        $this->assign('regis_sms_enable',$reg_sms_enable);
        return $this->fetch();
    }
	 ...
	 * 鱼分转帐
	**/
	public function score_zz(){
		if (IS_POST){
			$data = I('post.');
			$data['points'] = floatval($data['points']);
			if ($data['points'] <= 0){
				$this->error("数据异常");
                exit;
			}
			if ($this->user['pay_points'] <= 0){
				$this->error("帐户鱼分不足");
                exit;
			}
			if ($data['points'] > $this->user['pay_points']){
				$this->error("最多可转帐{$this->user['pay_points']}鱼分");
                exit;
			}
			...
			$userb = Db::name('users')->where('mobile', $data['mobile'])->find();
			if (!$userb){
				$this->error("对方帐号不存在！");
                exit;
			}
			...
			if ($data['mobile'] == $this->user['mobile']){
 			...
 			if (!(encrypt($data['password']) == $this->user['password'])){
				$this->error("您的密码不正确！");
				exit;
			}
			accountLog($this->user['user_id'], 0, -$data['points'], "鱼分转账(扣除)");
			accountLog($userb['user_id'], 0, $data['points'], "鱼分转账(到帐)");
			$this->success("鱼分转账成功");
		}
		$reg_sms_enable = tpCache('sms.regis_sms_enable');
		$this->assign('regis_sms_enable', $reg_sms_enable ? 1 : 0);
		$this->assign('user', $this->user);
		return $this->fetch();
	}
	...
	**/
	public function zz(){
		if (IS_POST){
			...
			$team_uids = find_all($members, $this->user['user_id']);
			if ($data['money'] <= 0){
				$this->error("数据异常");
                exit;
			}
			if ($this->user['user_money'] <= 0){
				$this->error("帐户余额不足");
                exit;
			}
			if ($data['money'] > $this->user['user_money']){
				$this->error("最多可转帐{$this->user['user_money']}元");
                exit;
			}
			if(!preg_match("/^1[345789]\d{9}$/", $data['mobile'])){
				$this->error("手机号格式不正确");
			...
			$userb = Db::name('users')->where('mobile', $data['mobile'])->find();
			if (!in_array($userb['user_id'], $team_uids)){
				$this->error("只能向团队下级转帐！");
                exit;
			}
			if (!$userb){
				$this->error("下级帐号不存在！");
			...
			accountLog($this->user['user_id'], -$data['money'], 0, "余额转账(扣除)");
			accountLog($userb['user_id'], $data['points'], 0, "余额转账(到帐)");
			$this->success("转账成功");
		}
		$reg_sms_enable = tpCache('sms.regis_sms_enable');
		$this->assign('regis_sms_enable', $reg_sms_enable ? 1 : 0);
	...
	public function sign()
	{
		if(IS_POST){
			$day = I('post.day');
			$month_sign = $this->getMonthSign();
			if (!intval($day)) return json_encode(['code'=>0, 'msg'=>"0"]);
			if ($day <> date('j') && in_array($day, $month_sign)) return json_encode(['code'=>0, 'msg'=>"该日期已经签到过！"]);
			if ($day <> date('j')) return json_encode(['code'=>0, 'msg'=>"暂不支持补签！"]);
			$todayData = $this->todayData();
			if($todayData['is_sign'] == 1){
				return json_encode(['code'=>0, 'msg'=>"今日已经签到过！"]);
			}else{
				...
				if($todayData == NULL){
					...
					$save = M('sign')->where('id', $todayData['id'])->save($data);
				}
				if($id or $save){
					accountLog($this->user_id, 0, 1, '签到赠送鱼分');
					return json_encode(['code'=>1, 'msg'=>"恭喜您，签到成功!系统赠送您1鱼分"]);
				}else{
					return json_encode(['code'=>0, 'msg'=>"签到失败，请刷新后重试！"]);
				}
			}
		}else{
			$this->assign('data', $this->endResult($this->getMonthSign()));
			$this->assign('today', date('j'));
			return $this->fetch();
		}
	}
	
	protected function getInsertData($user_id){
		$start_time = strtotime(date('Y-m-d 0:0:0',time()-86400))-1;
		$end_time  = strtotime(date('Y-m-d 23:59:59',time()-86400))+1;
		$days = M('sign')->where("user_id = $user_id and create_time > $start_time and create_time < $end_time")->getField('days');
		if($days){
			$days++;
			if($days > 30){
				$days = 1;
	...
	protected function todayData(){
		...
		return M('sign')->field('create_time', true)->where("user_id = {$this->user_id} and create_time > $start_stime and create_time < $end_stime")->find();
	}
	...
	protected function getMonthSign(){
		...
		$list = M('sign')->where("user_id = {$this->user_id} and sign_time > $start_stime and sign_time < $end_stime")->order('sign_time asc')->getField('sign_time',true);
		foreach ($list as $key => $value){
			$arr[$key] = date('j',$value);
		}
		return $arr;
	}
	
	protected function endResult($arr){
		$str = "[";
